...
void D_IdentifyVersion(void)
{
    ...
    LOGD("E4M1 lump index: %d", W_CheckNumForName("E4M1"));
    LOGD("E3M1 lump index: %d", W_CheckNumForName("E3M1"));
    LOGD("gamemode set to: %d", gamemode);
    if (gamemission == none)
    {
        unsigned int i;
    ...
    if (logical_gamemission == doom)
    {
        ...
        if (W_CheckNumForName("E4M1") > 0 && W_CheckNumForName("M_EPI4") > 0)
        {
            // Ultimate Doom
    ...
    for (i=0; gameversions[i].description != NULL; ++i)
    {
        if (gameversions[i].version == gameversion)
        {
            printf("Emulating the behavior of the "
                   "'%s' executable.\n", gameversions[i].description);
            LOGD("Emulating the behavior of the "
                   "'%s' executable.\n", gameversions[i].description);
            break;
        }
    }
